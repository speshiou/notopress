...
jest.mock('@/lib/render-rich-text', () => ({
  ...
  renderRichText: jest.fn((richTextArray) => {
    ...
    return richTextArray.map((rt: RichTextItemResponse) => rt.plain_text).join('');
  }),
}));
...
describe('CodeBlock Component', () => {
  ...
  test('displays the code content processed by renderRichText', () => {
    ...
    expect(screen.getByRole('code')).toHaveTextContent('console.log("Hello, Code!");');
  });
  ...
  test('does not display language or caption if not provided', () => {
    ...
    expect(screen.queryByText('This is a code caption.')).not.toBeInTheDocument();
